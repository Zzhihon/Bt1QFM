...
func (h *APIHandler) UploadTrackHandler(w http.ResponseWriter, r *http.Request) {
	...
	trackID, err := h.repo.CreateTrack(newTrack)
	if err != nil {
		// Check if the error is due to UNIQUE constraint violation on file_path
		if strings.Contains(strings.ToLower(err.Error()), "unique constraint") || strings.Contains(strings.ToLower(err.Error()), "duplicate entry") {
			http.Error(w, fmt.Sprintf("Failed to create track: A track resulting in the filename '%s' already exists. Error: %v", trackStoreFileName, err), http.StatusConflict)
		} else {
	...
	if track.HLSPlaylistPath == "" {
		...
		if track.Duration == 0 {
			...
			if durErr == nil && duration > 0 {
				if errDb := h.repo.UpdateTrackHLSPath(trackID, m3u8ServePath, duration); errDb != nil {
